...
public class GetWuLiuMsg {
	 ...
	 *
	 * @param context  上下文
	 * @param content  物流信息内容
	 * @param time     物流信息时间
	 * @param position 位置
	 * @param flag     是否为最后一个信息
	 * @return
	 */
	public static RelativeLayout getMsg_wuliu(final Context context, String content, String time, int position, boolean flag) {
		/**
		 *搭建父容器
		 */
		...
		img_0.setId(id_img0);
		RelativeLayout.LayoutParams p_img0 = new RelativeLayout.LayoutParams(dip2px(context, 1), dip2px(context, 22));//原：28大----现：22中----------------
		if (position == 0) {
			p_img0 = new RelativeLayout.LayoutParams(dip2px(context, (float) 0.3), dip2px(context, 18));//原：24大----现：18中-----------------------------
		}
		...
		img_2.setId(id_img2);
		RelativeLayout.LayoutParams p_img2 = new RelativeLayout.LayoutParams(dip2px(context, 1), dip2px(context, 56));//原：70大----现：56中------------------
		if (position == 0) {
			p_img2 = new RelativeLayout.LayoutParams(dip2px(context, 1), dip2px(context, 46));//原：60大----现：46中------------------------------------------
		}
		...
		p_place.topMargin = dip2px(context, 16);
		tv_place.setLayoutParams(p_place);
		tv_place.setText(content);
